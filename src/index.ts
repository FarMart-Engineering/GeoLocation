...
import { Geolocation, PincodeDumpObject, ReverseGeocoded, ClosenessObject } from "./interfaces/geolocationObject";
...
class GeoLocationDump {
    ...
    public reverseGeocode(latitude: number, longitude: number): ReverseGeocoded {
        try {
            let closenessScores: ClosenessObject[] = this.pincodeDetails.map((pincodeDetail: Geolocation, index: number) => {
            ...
            let closest = { ...this.pincodeDetails[closenessScores[0].index], differenceInKm: Number.MAX_VALUE };
            const differenceInKm = differenceBetweenCoordinates(latitude, longitude, closest.lat, closest.long);
